...
for (ii in 1:length(dend$tree_tag)) {  
  ...
  dend$agC[ii] <- (bm)*(1/(2.1097*1000)) # convert kg to Mg=1/1000=10 and convert to carbon = 47.8% ######### PROBLEM HERE!!!! SEE WARNINGS!!!!!!
  dend$bm_kg[ii] <- (bm)
}
...
colnames(npp_tree) <- c("plot_code", "tag", "year", "month", "agC", "agCdiff", "datediff")
  ...
  www$npp_avgplot_yr_dend   <- www$npp_avgtrees_yr_dend*length(unique(dend$tree_tag))
  ...
  nppacw_dend    <- avg(www$npp_avgplot_yr_dend)
  nppacw_census  <- 
  scaling_factor <- nppacw_dend / nppacw_census
  npp_plot_month <- (www$npp_avgtrees_day_dend*29.6)/scaling_factor

  # NPP per plot (average NPP per tree * number of trees in the plot)
  nppacw1                   <- sqldf("SELECT plot_code, year, month, AVG(nppacw_tree_day) FROM npp_tree GROUP BY year, month")
  colnames(nppacw1)         <- c("plot_code", "year", "month", "npp_avgplot_day")
  nppacw1$npp_avgplot_day   <- as.numeric(nppacw1$npp_avgplot_day)
  nppacw1$npp_avgplot_month <- (nppacw1$npp_avgplot_day*29.6)*length(cen$tree_tag)
  ...
  nppyr <- sqldf("SELECT plot_code, year, AVG(npp_MgC_month)*12 FROM monthlynppacw GROUP BY year")

  return(monthlynppacw)
}

# NPPacw per plot - sum of NPP per tree doesn't work?
# nppacw <- sqldf("SELECT plot_code, year, month, SUM(nppacw_tree_mo) FROM npp GROUP BY year, month") #AVG(density), AVG(height_m) 
# colnames(nppacw) <- c("plot_code","year","month","nppacw_MgC_month") 
