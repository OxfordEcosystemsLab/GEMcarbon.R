...
#!!!Code has been altered to keep datapoint values that are 0!!!
#!!!Code has been altered to preserve class column!!!

###################################################################

#Set input datafile
inputfn = "M:\\TimeSeries\\InputDataFiles\\BOB01_anomalies_diameters.csv"
#Set column indices:
ip = 4 #Plot name
id = 21 #Data points for plotting
ie = 0 #Error on data points (if no error column set ie=0)
it = 15 #Time (ie. date column)
ic = 27 #Class column
...
#Set output filename
outputstem = "M:\\TimeSeries\\Dendrometers\\BOB-01"
...
#Set yaxis label
ylabel = "Growth - Monthly Means (mm month^-1)"
#Set xaxis label
xlabel = 'Date'
    ...
    for (i in 2:r1)
        {if (as.character(f1[i,ip])==plots[j,1])
            {if ((is.na(f1[i,id])==FALSE)&(is.na(f1[i,it])==FALSE))
    	          {if ((as.character(f1[i,id])!="")&(as.character(f1[i,it])!='0')&(as.character(f1[i,it])!=""))
...
for (j in 1:nplots)
    ...
    d = data.frame(Date=as.Date(character(nd),format="%Y-%m-%d"),Y=numeric(nd),Y_err=numeric(nd),Y_errpos=numeric(nd),Y_errneg=numeric(nd),Month=integer(nd),class=character(nd),stringsAsFactors=FALSE)
        ...
        {if (as.character(f1[i,ip])==plots[j,1])
            {if ((is.na(f1[i,id])==FALSE)&(is.na(f1[i,it])==FALSE))
    	          {if ((as.character(f1[i,id])!="")&(as.character(f1[i,it])!='0')&(as.character(f1[i,it])!=""))
                    ...
                    d[counter,7] = as.character(f1[i,ic])
