...
#Code to read in all datafiles in a folder and plot the mean timeseries per class. 

###################################################################

#Set input directory
dirpath = "M:\\TimeSeries\\Dendrometers\\BOBAllRawData\\"
...
delta = 90 #ie. width of bins in days

#Vector containing class types to average over:
classes = c("all","under20cm","20to40cm","40to50cm","50cmplus")

#Set output filename for pdf
pdf("M:\\TimeSeries\\Dendrometers\\BOBMeanRawData.pdf")
#Set output filename for dataframe
outputcsv = "M:\\TimeSeries\\Dendrometers\\meanrawdata_BOB.csv"

#Set yaxis label
ylabel = "Mean Growth (mm day^-1)"
#Set xaxis label
xlabel = 'Date'
...
if (ie>0)
    {d = data.frame(dates=as.Date(fi[,it]),Y=as.numeric(fi[,id]),Yerr=as.numeric(fi[,ie]),fno=numeric(length(fi[,it])),bin=numeric(length(fi[,it])),class=as.character(fi[,ic]),stringsAsFactors=FALSE)
}
if (ie==0)
    {d = data.frame(dates=as.Date(fi[,it]),Y=as.numeric(fi[,id]),Yerr=as.numeric(length(fi[,it])),fno=numeric(length(fi[,it])),bin=numeric(length(fi[,it])),class=as.character(fi[,ic]),stringsAsFactors=FALSE)
}
#Fill in fno
...
for (i in 2:nfiles)
    {fi = read.csv(paste(dirpath,filelist[i],sep=""),header=FALSE,stringsAsFactors=FALSE)
	  if (ie>0)
        {dtemp = data.frame(dates=as.Date(fi[,it]),Y=as.numeric(fi[,id]),Yerr=as.numeric(fi[,ie]),fno=numeric(length(fi[,it])),bin=numeric(length(fi[,it])),class=as.character(fi[,ic]),stringsAsFactors=FALSE)
    }
    if (ie==0)
        {dtemp = data.frame(dates=as.Date(fi[,it]),Y=as.numeric(fi[,id]),Yerr=as.numeric(length(fi[,it])),fno=numeric(length(fi[,it])),bin=numeric(length(fi[,it])),class=as.character(fi[,ic]),stringsAsFactors=FALSE)
    }
    #Fill in fno
...
for (z in 1:length(classes))
 {print(paste("Class:",classes[z]))
 if (classes[z]=="all")
     {d = d0
 }
 else
     {d = subset(d0,class==classes[z])
 }
 #print(d)

 #Get binned dates
 binned_d = data.frame(dates=integer(length(d$dates)),Y=numeric(length(d$dates)),Yerr=numeric(length(d$dates)),N=numeric(length(d$dates)),class=as.character(length(d$dates)),stringsAsFactors=FALSE)
 starti = 1
 bin_no = 1
 ...
 #Make plot
 plot(binned_d$dates,binned_d$Y,pch=19,col='black',xlab="Date",ylab=ylabel,xlim=xlimits,ylim=ylimits,main=classes[z])
 points(d$dates,d$Y,col='grey')
 lines(d$dates,d$Y,col='grey')
 points(binned_d$dates,binned_d$Y,col='black',pch=19)
