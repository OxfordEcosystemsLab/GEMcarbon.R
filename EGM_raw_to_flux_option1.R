...
# simple steps:
# 1. organise respiration data into three files: total, partitionning and control (keep same numbers for ";Plot" column). Save your files as .csv
# 2. change directory in line 25 and replace "T_S_Resp_Lp1_01.07.2013.csv" with the name of the file you want to run this code through (same for controla nd partitionning)
...
# select a plot
  raw_totsr <- subset(raw_totsr, plot_code=="ACJ")
  wea_tot <- subset(data.frame(wea_tot), plot_code=="ACJ")
  
# Add air temperature (temp), volumetric water content (vwc), and chamber depth (depth) to the raw control data (raw_consr)
  wea_tot$code1     <- paste(wea_tot$subplot, wea_tot$day, wea_tot$month, wea_tot$year, sep=".") # only use code 1 to merge wea_tot and raw_totsr (subplot.day.month.year is not a unique identifier).
  raw_totsr$code1   <- paste(raw_totsr$subplot, raw_totsr$day, raw_totsr$month, raw_totsr$year, sep=".")
  raw_totsr         <- sqldf("SELECT raw_totsr.*, wea_tot.soil_temp, wea_tot.vwc, wea_tot.collar_depth FROM  raw_totsr JOIN  wea_tot  ON raw_totsr.code1 = wea_tot.code1")
